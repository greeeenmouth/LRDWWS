...
if __name__ == '__main__':
    ...
    dict_table = None
    if args.dict_file:
        dict_table = read_dict(args.dict_file)

    with open(args.text_file, 'r', encoding='utf-8-sig') as fin, \
         open(args.output_file, 'w', encoding='utf8') as fout:
        for line in fin:
            arr = line.strip().split(maxsplit=1)
            key = arr[0]
            if arr[1] in dict_table:
                txt = dict_table[arr[1]]
            else:
                txt = -1
