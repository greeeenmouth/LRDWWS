...
def add_white_noise(data, snr_range=[0, 15], noise_prob=0.5):
    for sample in data:
        assert 'wav' in sample
        assert 'key' in sample
        if noise_prob > random.random():
            ...
            noise_snr = random.uniform(snr_range[0], snr_range[1])
            snr = 10**(noise_snr/10)
            power = np.mean(np.square(audio))
            npower = power/snr
            noise_audio = np.float32(np.random.rand(audio_len)*np.sqrt(npower))
            out_audio = audio + noise_audio
            out_audio = torch.from_numpy(out_audio)
            out_audio = torch.unsqueeze(out_audio, 0)
            sample['wav'] = out_audio
