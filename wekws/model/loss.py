...
def acc_utterance(logits: torch.Tensor, target: torch.Tensor,
    ...
    for i in range(logits.size(0)):
        ...
        # lab:['2537', '2476', '1082', '1553']
        # rec:['2537', '2476', '1082', '1553']
        # result: {'lab': ['2537', '2476', '1082', '1553'],
        #          'rec': ['2537', '2476', '1082', '1553'],
        #          'all': 4,
        #          'cor': 4,
        #          'sub': 0,
...
def ctc_loss(logits: torch.Tensor,
    ...
    # logits: [bsz, frame, num_keywords] -> [256,211,2599]
    logits = logits.transpose(0, 1) # logits: (B, L, D) -> (L, B, D)
    # logits: [frame, bsz, num_keywords] -> [211,256,2599]
    ...
    for i in range(num_utts):
        for j in range(num_keywords):
            ...
            if target[i] == j:
                ...
                prob = torch.clamp(prob, 1e-8, 1.0)
                max_prob = prob.max()
                loss += - alp * ((1-max_prob)**gamma) * torch.log(max_prob)
            else:
                # For other keywords or filler, do min-polling
                prob = 1 - logits[i, :, j]
                prob = prob.masked_fill(mask[i], 1.0)
                prob = torch.clamp(prob, 1e-8, 1.0)
                min_prob = prob.min()
                loss += - (1-alp) * ((1-min_prob)**gamma) * torch.log(min_prob)
    loss = loss / num_utts
...
def criterion(type: str,
    ...
    if type == 'ce':
        ...
        return loss, acc
    elif type == 'max_pooling': #logits[5,724,10],  target:[-1,-1,2,3,1]
        ...
        loss, acc = focal_loss(logits, target, lengths, min_duration)
        return loss, acc
    else:
        exit(1)
